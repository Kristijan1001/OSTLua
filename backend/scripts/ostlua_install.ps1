...
try {
    $cfg     = Get-Content (Join-Path $PluginDir "backend\config.json") -Raw | ConvertFrom-Json
    $apiKey  = $cfg.api_key
        ...
        try {
            ...
            if ($isZip) {
                ...
                Get-ChildItem $ext -Recurse -Filter *.lua | ForEach-Object {
                    $dest = Join-Path $luaDir $_.Name; Copy-Item $_.FullName $dest -Force; $result.lua = $dest
                }
